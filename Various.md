...
See also [Linux dir colors](http://aplawrence.com/Linux/linux_dir_colors.html).
...
at> touch ~/a
at> <EOF>
job 4 at Mon Jan  5 08:40:00 2015
```
...
```
See also
- [The at command](http://www.brunolinux.com/02-The_Terminal/The_at_Command.html)
- [BLCMD1: at](http://linux.about.com/library/cmd/blcmdl1_at.htm)
...
- `[` and `]`     -- 'scroll' throughout the page horizontally
...
See also
- [Command the web, an elinks tutorial](http://zeth.net/archive/2007/07/25/command-the-web-an-elinks-tutorial/)
- [Manual](http://elinks.or.cz/documentation/manual.html)
...
- `-L 1` invoke a command for each line
  ...
  ```
- `-p` / `--interactive` prompts the user for confirmation
...
- [10 xargs command examples](http://javarevisited.blogspot.de/2012/06/10-xargs-command-example-in-linux-unix.html)
- [Xargs examples](http://www.thegeekstuff.com/2013/12/xargs-examples/)
- [Xargs](http://unixcommandstutorial.blogspot.de/2012/01/xargs.html)
...
Add music: go to browser view (`5`) and add to library (`a`)
...
downloads the page `dynamic.html` and all the objects it links to (`-r`/`--recursive`) up to 2 levels (`-l 2`) and with their requirements, e.g. included images (`-p`).

A simple `-r` would contain `-l 5`, and not requirements, so the 5th link images will be missing.

See also [Manual: Recursive retrieval options](https://www.gnu.org/software/wget/manual/wget.html#Recursive-Retrieval-Options)
...
echo $(($RANDOM%6+1)) 
```
See [Shell script generationg random numbers](http://www.cyberciti.biz/faq/bash-shell-script-generating-random-numbers/)
...
See [pastebin](http://pastebin.com/z9f422Sk).
...
allows to work with filenames which spaces
